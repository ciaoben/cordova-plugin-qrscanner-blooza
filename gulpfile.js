...
gulp.task(
  "webpack-cordova",
  gulp.series("prepack", function (cb) {
...
gulp.task(
  "remap",
  gulp.series("webpack-cordova", function (cb) {
    return gulp
      .src(["dist/plugin.min.js", "dist/www.min.js"])
...
gulp.task(
  "plugin",
  gulp.series("remap", function (cb) {
    return gulp.src(["dist/plugin.min.js"]).pipe(gulp.dest("src/browser"));
  })
);
...
gulp.task(
  "www",
  gulp.series("remap", function (cb) {
    return gulp.src(["dist/www.min.js"]).pipe(gulp.dest("www"));
  })
);

exports.default = series("dist", "plugin", "www");
